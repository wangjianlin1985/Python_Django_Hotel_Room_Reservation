				...
				if ($("#bookOrderEditForm").form("validate")) {
					...
					if(!$("#bookOrderEditForm").form("validate")) {
						...
						$("#bookOrderEditForm").form({
						    url:"/BookOrder/update/" + $("#bookOrder_orderId_edit").val(),
						    onSubmit: function(){
								if($("#bookOrderEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
...
function initBookOrderManageTool() {
	bookOrder_manage_tool = {
		...
		search: function() {
			...
			queryParams["leaveDate"] = $("#leaveDate").datebox("getValue"); 
		...
		exportExcel: function() {
			$("#bookOrderQueryForm").form({
			    url:"/BookOrder/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/BookOrder/update/" + rows[0].orderId,
					type : "get",
					data : {
					...
					success : function (bookOrder, response, status) {
						...
						if (bookOrder) { 
							...
							$("#bookOrder_leaveDate_edit").datebox({
							...
							$("#bookOrder_telephone_edit").val(bookOrder.telephone);
							$("#bookOrder_telephone_edit").validatebox({
								required : true,
