			...
			handler : function () {
				if ($("#roomEditForm").form("validate")) {
					...
					if(!$("#roomEditForm").form("validate")) {
						...
						$("#roomEditForm").form({
						    url:"/Room/update/" + $("#room_roomNo_edit").val(),
						    onSubmit: function(){
								if($("#roomEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
...
function initRoomManageTool() {
	room_manage_tool = {
		...
		exportExcel: function() {
			$("#roomQueryForm").form({
			    url:"/Room/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
				...
				$.ajax({
					...
					success : function (room, response, status) {
						...
						if (room) { 
							...
							$("#room_area_edit").val(room.area);
							$("#room_area_edit").validatebox({
								required : true,
								validType : "number",
								missingMessage : "请输入面积",
								invalidMessage : "面积输入不对",
							});
							tinyMCE.editors['room_roomDesc_edit'].setContent(room.roomDesc);
						} else {
